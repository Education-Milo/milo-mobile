...
import { generateExercise, generateQCM, startDocumentChat } from "@features/ocr/api/ocr.api";
...
const extractExerciseStatement = (data: any) => {
...
const extractExerciseCorrection = (data: any) => {
  const reply = data?.reply ?? data;
  const correction =
    reply?.correction ??
    reply?.solution ??
...
export const useScanOrImportDocument = ({
  ...
  const processImage = async (action: OcrProcessingAction) => {
    if (!selectedDocument) return;
  ...
  const handleConfirmSend = async (action?: OcrProcessingAction) => {
    if (!selectedDocument) return;
    ...
    try {
      const data =
        documentType === "bulletin"
          ...
          : action
            ? await processImage(action)
            : undefined;
      setShowSendModal(false);

      if (documentType === "bulletin") {
        showMessage({
          message: "Votre bulletin a bien été envoyé à Milo.",
          type: "success",
        });
      ...
      if (documentType === "exercice" && action === "exercise" && data) {
        navigation.navigate("GeneratedExerciseScreen", {
          ...
          correction: extractExerciseCorrection(data),
