...
import { useSubjects } from "@features/course/api/course.queries";
import { CLASS_OPTIONS } from "@shared/constants/constants";
...
export const useLessonScreen = () => {
	...
	const availableClasses = CLASS_OPTIONS.filter((option) => {
		const userClassIndex = CLASS_OPTIONS.findIndex(
			(o) => o.value === user?.classe,
		);
		const optionIndex = CLASS_OPTIONS.findIndex(
