...
import {
	ChapterWithLessons,
	LessonWithStatus,
} from "@features/course/hooks/useLessonChapterScreen";
...
const ChapterAccordion = ({
	...
	const getLessonIcon = (status: string) => {
		switch (status) {
			...
			case "in-progress":
				return (
					<Ionicons name="play-circle-outline" size={18} color="#FF9800" />
				);
			default:
				return (
				...
				<View style={styles.chapterContent}>
					{chapter.lessons.map((lesson) => (
						<TouchableOpacity
							...
							style={[
								styles.lessonItem,
								styles[`lesson_${lesson.status}` as keyof typeof styles],
							]}
							onPress={() =>
								lesson.status !== "locked" && onLessonPress(lesson)
							}
